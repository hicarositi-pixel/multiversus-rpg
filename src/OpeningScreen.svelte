<script>
    ...
    // Importante: Assegura que está lendo as configurações registradas no main.js
    ...
    // Controle de Áudio
    let bgAudio = null;
    let isAudioBlocked = false; // Controle para mostrar botão de desmutar
    ...
    // --- CICLO DE VIDA ---
    onMount(() => {
        // Ajuste CSS forçado para garantir tela cheia
        if (app?.element) {
        ...
        if (VIDEO_PATH && !skipVideoPref) {
            state = 'video';
            // Tenta iniciar música junto com vídeo (opcional, dependendo se o vídeo tem som próprio)
            // Se o vídeo já tem som, talvez não queira music_path aqui. 
            // Mas para garantir som ambiente:
            if(MUSIC_PATH) startMusic(); 
    ...
    function startMusic() {
        ...
        if (bgAudio) return; // Já existe

        bgAudio = new Audio(MUSIC_PATH);
        bgAudio.loop = true;
        bgAudio.volume = 0.5; // Volume inicial

        // Tenta tocar. Se der erro (bloqueio), mostra botão.
        bgAudio.play().then(() => {
            isAudioBlocked = false;
        }).catch(e => {
            console.warn("Autoplay bloqueado. Aguardando interação.", e);
            isAudioBlocked = true;
        });
    }
    ...
    function unlockAudio() {
        if (bgAudio) {
            ...
            // Caso tenha clicado antes de iniciar a musica
            startMusic();
        }
    }
    ...
    async function runTerminalTyping() {
        ...
        await delay(500);
        terminalLines = [...terminalLines, "NEXUS_OS_BOOTLOADER_V2.4..."]; await delay(400);
        terminalLines = [...terminalLines, "Estabelecendo link neural..."]; await delay(600);
        terminalLines = [...terminalLines, "Buscando credenciais..."]; await delay(600);
        
        terminalLines = [...terminalLines, `> USER: ${playerName}`]; await delay(800);
...
<div class="main-container" style="--primary: {theme.colors.primary}; --bg-grad: {theme.colors.bg}; --font: {theme.font}">
    ...
    {#if isAudioBlocked}
        <button class="unmute-overlay" on:click={unlockAudio} transition:fade>
            <i class="fas fa-volume-mute"></i> ATIVAR SISTEMA DE SOM
        </button>
    {/if}
    ...
    {#if state === 'video'}
        <div class="fullscreen-layer bg-black" out:fade={{duration: 1000}}>
            <video 
                ...
                on:ended={endVideo}
                on:click={endVideo}
                on:error={() => { console.warn("Erro vídeo"); endVideo(); }}
            ></video>
    ...
    {#if state === 'login'}
        <div class="fullscreen-layer bg-black terminal-layout scanlines" out:fade={{duration: 500}}>
            <div class="terminal-box">
                <div class="term-header">TERMINAL SECURE SHELL v2.4</div>
        ...
        <div class="fullscreen-layer main-menu" in:fly={{y: 50, duration: 1500, opacity: 0}}>
            ...
            <div class="menu-options">
                <button class="menu-btn primary" on:click={enterGame}>
                    <i class="fas fa-power-off"></i> <span>INICIAR SESSÃO</span>
                </button>
                <button class="menu-btn locked" on:click={lockedAction}>
                    <i class="fas fa-newspaper"></i> <span>JORNAL DO NEXUS</span> <span class="tag">OFFLINE</span>
                </button>
                <button class="menu-btn locked" on:click={lockedAction}>
                    <i class="fas fa-user-secret"></i> <span>REDE SECRETA</span> <span class="tag">???</span>
                </button>
            </div>
    ...
    {#if state === 'entering'}
        <div class="fullscreen-layer entering-screen">
            <div class="welcome-msg">
                <span class="wel-sub">SINCRONIZANDO NEURAL LINK...</span>
                <span class="wel-main">BEM VINDO, {playerName.toUpperCase()}</span>
            </div>
            <div class="loading-bar-container"><div class="bar-fill"></div></div>
        </div>
    {/if}
...
<style>
    /* RESET GLOBAL PARA SOBREPOR O FOUNDRY */
    ...
    .main-container { width: 100%; height: 100%; font-family: var(--font, 'monospace'); color: #fff; background: radial-gradient(circle at center, #1a1a1a 0%, #000000 100%); overflow: hidden; position: relative; }
    .fullscreen-layer { position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 100%; }
    .bg-black { background: #000; }

    /* BOTÃO UNMUTE */
    .unmute-overlay {
        position: absolute; top: 20px; right: 20px; z-index: 10000;
        background: rgba(255, 0, 0, 0.8); border: 1px solid #fff; color: #fff;
        padding: 10px 20px; font-weight: bold; cursor: pointer;
        animation: pulseBtn 1.5s infinite; display: flex; align-items: center; gap: 10px;
    }
    @keyframes pulseBtn { 0% { opacity: 0.8; } 50% { opacity: 1; transform: scale(1.05); } 100% { opacity: 0.8; } }

    /* VÍDEO */
    .video-element { width: 100%; height: 100%; object-fit: cover; }
    .skip-btn { position: absolute; bottom: 40px; right: 40px; background: rgba(0,0,0,0.7); border: 1px solid #fff; color: #fff; padding: 12px 25px; cursor: pointer; font-family: inherit; font-size: 1rem; transition: 0.3s; z-index: 10; letter-spacing: 2px; }
    ...
    .term-header { border-bottom: 2px solid var(--primary); margin-bottom: 20px; padding-bottom: 10px; font-weight: bold; color: var(--primary); font-size: 1.5rem; letter-spacing: 2px; }
    ...
    .scanlines::before { content: " "; display: block; position: absolute; top: 0; left: 0; bottom: 0; right: 0; background: linear-gradient(rgba(18, 16, 16, 0) 50%, rgba(0, 0, 0, 0.25) 50%), linear-gradient(90deg, rgba(255, 0, 0, 0.06), rgba(0, 255, 0, 0.02), rgba(0, 0, 255, 0.06)); z-index: 2; background-size: 100% 2px, 3px 100%; pointer-events: none; }
    .skip-text-btn { margin-top: 20px; background: transparent; border: 1px solid #444; color: #666; padding: 5px 10px; cursor: pointer; transition: 0.2s; z-index: 5; }
    .skip-text-btn:hover { color: #fff; border-color: #fff; }

    /* MENU */
    .main-menu { background: var(--bg-grad); gap: 5vh; }
    .logo-area h1 { font-size: 5rem; margin: 0; color: #fff; text-shadow: 0 0 20px var(--primary); text-transform: uppercase; line-height: 1; letter-spacing: 8px; animation: glitch 4s infinite; }
    ...
    .menu-btn { padding: 20px; background: rgba(20,20,20,0.9); border: 1px solid #444; color: #ccc; cursor: pointer; font-family: inherit; font-size: 1.3rem; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: flex; align-items: center; justify-content: space-between; clip-path: polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px); }
    ...
    .settings-modal { position: absolute; bottom: 80px; background: rgba(10,10,10,0.95); border: 1px solid var(--primary); padding: 25px; width: 350px; border-radius: 4px; z-index: 20; box-shadow: 0 0 50px rgba(0,0,0,0.8); backdrop-filter: blur(10px); }
    ...
    @keyframes fadeOutScreen { 0% { opacity: 1; } 85% { opacity: 1; } 100% { opacity: 0; pointer-events: none; } }
    @keyframes glitch { 0%, 100% { transform: translate(0); opacity: 1; } 1% { transform: translate(-2px, 1px); opacity: 0.9; } 2% { transform: translate(2px, -1px); opacity: 1; } }
</style> 