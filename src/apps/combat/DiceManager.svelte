        ...
        {#each rolledDice as die, i}
            <div class="die {die.type}" class:wiggle={die.val === 0}>
                {#if die.type === 'wd' && die.val === 0}
                    <select on:change={(e) => updateWiggle(i, e.target.value)} disabled={!isOwner}>
...
<style>
    ...
    .die { width: 45px; height: 45px; background: #111; border: 2px solid #444; border-radius: 6px; display: flex; justify-content: center; align-items: center; font-size: 22px; font-weight: bold; color: #fff; box-shadow: inset 0 0 10px rgba(0,0,0,0.8), 0 5px 10px rgba(0,0,0,0.5); transition: 0.2s; font-family: 'Share Tech Mono', monospace; }
    ...
    .die.wiggle { animation: pulseWiggle 1.5s infinite alternate; }
    
    .die select { width: 100%; height: 100%; background: transparent; border: none; color: #fc0; font-weight: bold; text-align: center; font-size: 20px; outline: none; cursor: pointer; -webkit-appearance: none; font-family: inherit; }
    .die select:disabled { cursor: not-allowed; opacity: 0.8; }
    ...
    @keyframes pulseWiggle { from { box-shadow: inset 0 0 10px rgba(0,0,0,0.8), 0 0 5px #fc0; border-color: #aa8800; } to { box-shadow: inset 0 0 10px rgba(0,0,0,0.8), 0 0 20px #fc0; border-color: #fc0; } }
</style>