<script>
    ...
    // Sincronia local
    let baseBio = group.bio || { fome: 10, sede: 10, exaustao: 0 };
    let baseStats = group.baseStats || { hp: 20, maxHp: 20, har: 0, lar: 0, protection: 1 };

    // --- CÁLCULO DE CONSUMO ---
    // 1 NPC = 1 Suprimento por Ação (Regra do prompt)
    // O custo total do ciclo (2 ações) seria npcCount * 2.
    // Mas aqui mostramos "Por Ação" conforme pedido.
    $: npcCount = group.npcs?.length || 0;
    $: costPerAction = npcCount; 
    $: totalCycleCost = costPerAction * 2; // Ciclo padrão são 2 ações/dia
    ...
    async function runCycle() {
        if (!isGM) return;
        // Chama a função da database que vai descontar 'totalCycleCost' do inventário
